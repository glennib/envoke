...
- **`main.rs`** -- CLI (clap), reads YAML config, orchestrates resolution, delegates output formatting to `render.rs` or exec to `exec.rs`. Supports `--output` file writing, `--format` (curated presets: shell, shell-export, dotenv, json, yaml, k8s-secret, github-actions, terraform-tfvars), `--prepend-export` (legacy alias of `--format shell-export`), `--template` (custom output template), `--tag` (conditional inclusion), `--override` (per-variable source overrides), `--schema` (JSON Schema output), `--completions` (shell completions via `clap_complete`), and the trailing exec form `envoke <env> -- <cmd>` (requires `--` separator; conflicts with the output-shaping flags).
